...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!association) {
      return NextResponse.json({ error: 'Process type association not found' }, { status: 404 });
    }
    ...
    if (body.licenseTypeId && body.processTypeId) {
      ...
      if (existingAssociation) {
        return NextResponse.json(
          {
            error:
              'An active association between this license type and process type already exists',
          },
          { status: 409 },
        );
      }
    ...
    if (body.executionOrder && body.licenseTypeId) {
      ...
      if (duplicateOrder) {
        return NextResponse.json(
          {
            error:
              'An active process with this execution order already exists for this license type',
          },
          { status: 409 },
        );
      }
