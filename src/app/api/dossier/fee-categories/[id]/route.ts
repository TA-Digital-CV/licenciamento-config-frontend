...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (body.code && body.code !== currentCategory.code) {
      const existingCategory = mockFeeCategories.find(
        (cat) => cat.id !== id && cat.code === body.code && cat.active,
      );
...
export async function DELETE(
  ...
  try {
    ...
    const feesUsingCategory = mockProcessTypeFees.filter(
      (fee) => fee.feeCategoryId === id && fee.active,
    );
    ...
    const childCategories = mockFeeCategories.filter(
      (cat) => cat.parentCategoryId === id && cat.active,
    );
