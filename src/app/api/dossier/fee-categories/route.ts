...
export async function GET(request: NextRequest) {
  try {
    ...
    // Filter data
    let filteredCategories = mockFeeCategories.filter(category => {
    ...
    filteredCategories.sort((a, b) => {
      let aValue: any = a[sortBy as keyof FeeCategoryRecord];
      let bValue: any = b[sortBy as keyof FeeCategoryRecord];
...
export async function POST(request: NextRequest) {
  try {
    ...
    const sortOrder = body.sortOrder !== undefined ? body.sortOrder : 
      Math.max(...mockFeeCategories
        .filter(cat => cat.level === body.level && 
                      cat.parentCategoryId === body.parentCategoryId)
        .map(cat => cat.sortOrder), 0) + 1;
