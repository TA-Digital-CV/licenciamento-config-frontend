import { NextRequest, NextResponse } from 'next/server';
import { apiClient } from '../../../../(myapp)/lib/api-client';
import { FeeCategoryResponseDTO } from '../../../../(myapp)/types/fee-categories.types';

// PATCH /api/fee-categories/[id]/deactivate
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    const { id } = await params;

    if (!id) {
      return NextResponse.json({ error: 'Fee category ID is required' }, { status: 400 });
    }

    // Call backend deactivation endpoint
    await apiClient.patch(`/fee-categories/${id}/desativar`);

    // Get updated fee category
    const response = await apiClient.get<FeeCategoryResponseDTO>(`/fee-categories/${id}`);

    return NextResponse.json({
      ...response,
      message: 'Fee category deactivated successfully',
    });
  } catch (error) {
    console.error('Error deactivating fee category:', error);
    return NextResponse.json({ error: 'Failed to deactivate fee category' }, { status: 500 });
  }
}
