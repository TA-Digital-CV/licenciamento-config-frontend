...
export default function PageDossierLicenceTypeComponent({
  params,
}: {
  ...
  const [validityUnitOptions, setValidityUnitOptions] = useState<
    { value: string; label: string }[]
  >([]);
  const [licensingModelOptions, setLicensingModelOptions] = useState<
    { value: string; label: string }[]
  >([]);
  const [legislationTypeOptions, setLegislationTypeOptions] = useState<
    { value: string; label: string }[]
  >([]);
  const [legislationStatusOptions, setLegislationStatusOptions] = useState<
    { value: string; label: string }[]
  >([]);
    ...
    async function load() {
      ...
      try {
        ...
        if (mounted) {
          ...
          const leg = Array.isArray(data?.metadata?.dossier?.legislations)
            ? data.metadata.dossier.legislations
            : [];
          setLegislations(leg);
          const ents = Array.isArray(data?.metadata?.dossier?.entities)
            ? data.metadata.dossier.entities
            : [];
          setEntities(ents);
          const procs = Array.isArray(data?.metadata?.dossier?.processTypes)
            ? data.metadata.dossier.processTypes
            : [];
        ...
        igrpToast({
          title: 'Erro',
          description: e?.message || 'Falha ao carregar tipo de licença',
          type: 'default',
        });
      } finally {
        setLoading(false);
    ...
    async function loadOptions() {
      try {
        const res = await fetch(
          `/api/options?codes=VALIDITY_UNIT,LICENSING_MODEL,TPLEG,LEGISLATION_STATUS,ENTITY_TYPE,PROCESS_TYPE,FEE_TYPE`,
          { signal: controller.signal },
        );
        if (!res.ok) throw new Error('Falha ao carregar opções');
        const data = await res.json();
        const sets = data?.optionSets || {};
        const validity = (sets?.VALIDITY_UNIT?.items || []).map((it: any) => ({
          value: it.key,
          label: it.value,
        }));
        const models = (sets?.LICENSING_MODEL?.items || []).map((it: any) => ({
          value: it.key,
          label: it.value,
        }));
        const legTypes = (sets?.TPLEG?.items || []).map((it: any) => ({
          value: it.key,
          label: it.value,
        }));
        const legStatus = (sets?.LEGISLATION_STATUS?.items || []).map((it: any) => ({
          value: it.key,
          label: it.value,
        }));
        const entityTypes = (sets?.ENTITY_TYPE?.items || []).map((it: any) => ({
          value: it.key,
          label: it.value,
        }));
        const processTypes = (sets?.PROCESS_TYPE?.items || []).map((it: any) => ({
          value: it.key,
          label: it.value,
        }));
        const feeTypes = (sets?.FEE_TYPE?.items || []).map((it: any) => ({
          value: it.key,
          label: it.value,
        }));
  ...
  const handleSaveGeneral = async (values: any) => {
    ...
    try {
      ...
      igrpToast({
        title: 'Sucesso',
        description: 'Dados Gerais guardados com sucesso',
        type: 'default',
      });
    } catch (e: any) {
      console.error(e);
      igrpToast({
        title: 'Erro',
        description: e?.message || 'Erro ao guardar Dados Gerais',
        type: 'default',
      });
    } finally {
  ...
  const handleSaveLegislation = async (values: z.infer<typeof legislationSchema>) => {
    ...
    try {
      ...
      igrpToast({
        title: 'Sucesso',
        description: 'Legislação guardada com sucesso',
        type: 'default',
      });
    } catch (e: any) {
      console.error(e);
      igrpToast({
        title: 'Erro',
        description: e?.message || 'Erro ao guardar Legislação',
        type: 'default',
      });
    } finally {
  ...
  const handleDeleteLegislation = async (originalIndex: number) => {
    ...
    try {
      ...
      igrpToast({
        title: 'Sucesso',
        description: 'Legislação removida com sucesso',
        type: 'default',
      });
    } catch (e: any) {
      console.error(e);
      igrpToast({
        title: 'Erro',
        description: e?.message || 'Erro ao remover Legislação',
        type: 'default',
      });
    } finally {
      setSavingLegislation(false);
  ...
  const handleSaveEntity = async (values: z.infer<typeof entitySchema>) => {
    ...
    try {
      ...
      igrpToast({
        title: 'Sucesso',
        description: 'Entidade guardada com sucesso',
        type: 'default',
      });
    } catch (e: any) {
      console.error(e);
      igrpToast({
        title: 'Erro',
        description: e?.message || 'Erro ao guardar Entidade',
        type: 'default',
      });
    } finally {
  ...
  const handleDeleteEntity = async (originalIndex: number) => {
    ...
    try {
      ...
      igrpToast({
        title: 'Erro',
        description: e?.message || 'Erro ao remover Entidade',
        type: 'default',
      });
    } finally {
      setSavingEntity(false);
  ...
  const handleSaveProcessType = async (values: z.infer<typeof processAssocSchema>) => {
    ...
    try {
      ...
      igrpToast({
        title: 'Sucesso',
        description: 'Associação guardada com sucesso',
        type: 'default',
      });
    } catch (e: any) {
      console.error(e);
      igrpToast({
        title: 'Erro',
        description: e?.message || 'Erro ao guardar Associação',
        type: 'default',
      });
    } finally {
  ...
  const handleDeleteProcessType = async (originalIndex: number) => {
    ...
    try {
      ...
      igrpToast({
        title: 'Erro',
        description: e?.message || 'Erro ao remover Associação',
        type: 'default',
      });
    } finally {
      setSavingProcess(false);
  ...
  const handleSaveFee = async (values: z.infer<typeof feeSchema>) => {
    ...
    try {
      ...
      igrpToast({
        title: 'Erro',
        description: e?.message || 'Erro ao guardar Taxa',
        type: 'default',
      });
    } finally {
      setSavingFee(false);
  ...
  const handleDeleteFee = async (originalIndex: number) => {
    ...
    try {
      ...
      igrpToast({
        title: 'Erro',
        description: e?.message || 'Erro ao remover Taxa',
        type: 'default',
      });
    } finally {
      setSavingFee(false);
