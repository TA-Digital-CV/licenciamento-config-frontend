...
export default function PageLicenciamentoParametrizacao() {
  ...
  const tabParam = (searchParams.get('tab') as TabId) || 'sectors';
  const active: TabId = (TABS.some((tab) => tab.id === tabParam) ? tabParam : 'sectors') as TabId;

  // Fallback para tab inválida: garante tab=sectors na URL
  useEffect(() => {
    const currentTab = searchParams.get('tab') as TabId | null;
    if (!currentTab || !TABS.some((t) => t.id === currentTab)) {
      const params = new URLSearchParams(searchParams.toString());
      params.set('tab', 'sectors');
      router.replace(`${pathname}?${params.toString()}`);
    }
  }, [searchParams, router, pathname]);

  // Nota: estado local removido; URL é a fonte de verdade.
    ...
    <div className="p-4">
      ...
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {active === 'sectors' && (
            <Link
              href="/parametrizacao/sectors/novo"
          ...
          {active === 'categories' && (
            <Link
              href="/parametrizacao/category/novo"
              className="inline-flex items-center rounded border px-3 py-1.5 text-sm hover:bg-accent"
            >
              <svg
                className="mr-2 h-4 w-4"
          ...
          {active === 'licence-types' && (
            <Link
              href="/parametrizacao/licence-type/novo"
              className="inline-flex items-center rounded border px-3 py-1.5 text-sm hover:bg-accent"
            >
              <svg
                className="mr-2 h-4 w-4"
          ...
          {TABS.map((tab) => (
            <button
              ...
              onClick={() => {
                ...
                // Opcional: limpar sectorId quando não estiver na aba de categorias
                // if (tab.id !== 'categories') params.delete('sectorId');
      ...
      <div className="mt-4">
        ...
        >
          <div className="rounded-lg border border-border bg-card p-4">
            ...
            <div>
              <Categorylist parentId={undefined as any} />
            </div>
          </div>
        </section>
